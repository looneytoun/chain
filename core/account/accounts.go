...
func (m *Manager) createControlProgram(ctx context.Context, accountID string, change bool, expiry time.Time) (*controlProgram, error) {
...
func (m *Manager) CreateControlProgram(ctx context.Context, accountID string, change bool, expiry time.Time) ([]byte, error) {
	cp, err := m.createControlProgram(ctx, accountID, change, expiry)
	if err != nil {
		return nil, err
	}
...
func (m *Manager) insertAccountControlProgram(ctx context.Context, progs ...*controlProgram) error {
	const q = `
		INSERT INTO account_control_programs (signer_id, key_index, control_program, change, expiry)
		SELECT unnest($1::text[]), unnest($2::bigint[]), unnest($3::bytea[]), unnest($4::boolean[]),
			unnest($5::timestamp without time zone[])
	`
	var (
	...
	for _, p := range progs {
		...
		expirations = append(expirations, p.expiry.Format(time.RFC3339))
	}

	_, err := m.db.Exec(ctx, q, accountIDs, keyIndexes, controlProgs, change, expirations)
